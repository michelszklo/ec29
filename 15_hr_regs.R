...
output <- function(y,transform,output_name,y0,yf,ys,graph_file){
  ...
  ggsave(paste0("hr/",graph_file,".png"),
         plot = plot,
         device = "png",
         width = 7, height = 5,
         units = "in")
  ggsave(paste0("hr/",graph_file,".pdf"),
         plot = plot,
         device = "pdf",
         width = 7, height = 5,
         units = "in")
...
graph_yaxis <- rbind(cbind('hr_all_pcapita','Total Health workers per 1000 population (asinh)'),
                     cbind('hr_superior_pcapita','Doctors per 1000 population (asinh)'),
                     cbind('hr_technician_pcapita','Health Technicians per 1000 population (asinh)'),
                     cbind('hr_elementary_pcapita','Elementary Health workers per 1000 population (asinh)'),
                     cbind('hr_admin_pcapita','Administrative workers per 1000 population (asinh)')
)
...
for(i in seq.int(1,5)){
  var <- graph_yaxis[i,1]
  output(var,1,var,-0.1,0.1,0.05,var)
}