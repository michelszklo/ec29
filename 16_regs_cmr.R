...
# April 2021
# 
# This scripts runs regressions for public Child Mortality rates
#
#
#######################################################################################################
...
var_map <-  rbind(cbind('tx_mc','Child Mortality Rate'),
...
if(below==1){
  write.xlsx2(df_table_all, file = paste0(dir,main_folder,output_file),sheetName = "cmr_b",row.names = F,append = T)
  
}else{
  
  write.xlsx2(df_table_all, file = paste0(dir,main_folder,output_file),sheetName = "cmr",row.names = F,append = T)
}
