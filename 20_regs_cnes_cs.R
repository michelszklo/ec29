...
df <- df %>% delta_2000_2005_cnes_merge() %>% 
  select(ano, cod_mun, uf, pop, finbra_desp_saude_san_pcapita, siops_despsaude_pcapita, 
         delta_finbra_desp_saude_san_pcapita, delta_siops_despsaude_pcapita, everything())

df_above <- df_above %>% delta_2000_2005_cnes_merge() %>% 
  select(ano, cod_mun, uf, pop, finbra_desp_saude_san_pcapita, siops_despsaude_pcapita, 
         delta_finbra_desp_saude_san_pcapita, delta_siops_despsaude_pcapita, everything())


df_below <- df_below %>% delta_2000_2005_cnes_merge() %>% 
  select(ano, cod_mun, uf, pop, finbra_desp_saude_san_pcapita, siops_despsaude_pcapita, 
         delta_finbra_desp_saude_san_pcapita, delta_siops_despsaude_pcapita, everything())
...
if(below==0){
  iv_first_cs(df_above,"delta_siops_despsaude_pcapita",1998,"table_above")
}
...
regress_output <- function(var,var_name,transform,year_filter){
  ...
  for (data in c("df","df_above","df_below")){
    ...
    iv_cs(var,"delta_siops_despsaude_pcapita",d,obj,transform,year_filter) # function for IV regression and bootstrap estimating of SE
  ...
  table_above_3 <- reg_df_above %>% mutate(sample = "above") %>% table_formating(3) %>% rename("2SLS_above" = "2SLS") %>% select(-term) %>% mutate(spec=3) %>% mutate(obs_above = obs_above_3)
  ...
  for (data in c("df","df_above","df_below")){
    ...
    d <- get(data)
    obj <- paste0("reg_",data) # name of the output object
    ols_cs(var,"delta_siops_despsaude_pcapita",d,obj,transform,year_filter) # function for OLS regression
