...
df <- df %>%
  filter(ano<=2010) %>%
  mutate(iv=ifelse(ano==2002,0,iv)) 
df_below <- df_below %>%
  filter(ano<=2010) %>%
  mutate(iv=ifelse(ano==2002,0,iv)) 
df_above <- df_above %>%
...
if(below==1){
  write.xlsx2(df_table_all, file = paste0(dir,main_folder,output_file),sheetName = "infra2_b",row.names = F,append = T)
  
}else{
  
  write.xlsx2(df_table_all, file = paste0(dir,main_folder,output_file),sheetName = "infra2",row.names = F,append = T)
}
