...
# April 2021
# 
# This scripts runs regressions for health infrastructure data
#
#
#######################################################################################################
...
load("regs.RData")
  ...
  select(ACS_popprop, ACS_I_pcapita, eSF_popprop, eSF_I_pcapita, leitos_pc, everything()) %>% 
  filter(leitos_pc==0) %>% 
  count
...
output <- function(y,transform,output_name,y0,yf,ys,graph_file){
  ...
  ggsave(paste0("infra/",graph_file,".png"),
         plot = plot,
         device = "png",
         width = 7, height = 5,
         units = "in")
  ggsave(paste0("infra/",graph_file,".pdf"),
         plot = plot,
         device = "pdf",
         width = 7, height = 5,
         units = "in")
...
graph_yaxis <-  rbind(cbind('ACS_popprop','Population covered (share) by Community Health Agents (asinh)'),
                      cbind('eSF_popprop','Population covered (share) by Family Health Agents (asinh)'),
                      cbind('d_hospital_mun','Presence of Municipal Hospital'),
                      cbind('unity_mun_pcapita','Municipal Outpatient Facilities per 1000 population (asinh)'),
                      cbind('leitos_pc','Hospital Beds per capita (asinh)'))
...
var <- graph_yaxis[1,1]
output(var,1,var,-0.3,0.3,0.1,var)

var <- graph_yaxis[2,1]
output(var,1,var,-0.3,0.3,0.1,var)

var <- graph_yaxis[3,1]
output(var,0,var,-0.3,0.3,0.1,var)
